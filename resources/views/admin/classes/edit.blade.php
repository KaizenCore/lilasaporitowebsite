            ...
            <div class="p-6">
                <form action="{{ route('admin.classes.update', $class) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Brief one-liner about the class (max 500 characters)</p>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="5" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('description') border-red-500 @enderror">{{ old('description', $class->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Materials Included -->
                    <div class="mb-4">
                        <label for="materials_included" class="block text-sm font-medium text-gray-700">Materials Included</label>
                        <textarea name="materials_included" id="materials_included" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('materials_included') border-red-500 @enderror">{{ old('materials_included', $class->materials_included) }}</textarea>
                        <p class="mt-1 text-sm text-gray-500">List what materials are provided</p>
                    ...
                    @if($class->image_path)
                        <div class="mb-4">
                            ...
                            <img src="{{ asset('storage/' . $class->image_path) }}" alt="{{ $class->title }}" class="mt-2 h-48 w-auto rounded-lg">
                        </div>
                    @endif
                    ...
                    <div class="mb-4">
                        <label for="image" class="block text-sm font-medium text-gray-700">
                            {{ $class->image_path ? 'Replace Image' : 'Class Image' }}
                        </label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <!-- Class Date -->
                        <div>
                            <label for="class_date" class="block text-sm font-medium text-gray-700">Class Date & Time *</label>
                            <input type="datetime-local" name="class_date" id="class_date" value="{{ old('class_date', $class->class_date->format('Y-m-d\TH:i')) }}" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('class_date') border-red-500 @enderror">
                            @error('class_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', $class->duration_minutes) }}" min="30" max="480" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('duration_minutes') border-red-500 @enderror">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', $class->location) }}" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('location') border-red-500 @enderror">
                        @error('location')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="price_cents" id="price_cents" value="{{ old('price_cents', $class->price_cents) }}" min="0" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('price_cents') border-red-500 @enderror">
                            <p class="mt-1 text-sm text-gray-500">Enter in cents (e.g., 5000 = $50.00)</p>
                            @error('price_cents')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $class->capacity) }}" min="1" max="100" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 @error('capacity') border-red-500 @enderror">
                            @error('capacity')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex items-center justify-end space-x-4 mt-6">
                        <a href="{{ route('admin.classes.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded">
                            Cancel
                        </a>
