    ...
    <div class="py-12 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            @if($featuredProducts->count() > 0 && !request()->filled('search') && !request()->filled('category'))
                <div class="mb-12">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Featured Products</h3>
                        ...
                        @foreach($featuredProducts as $product)
                            <a href="{{ route('store.show', $product->slug) }}" class="group">
                                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                                    ...
                                    @endif
                                    <div class="p-4">
                                        <h4 class="font-semibold text-gray-900 dark:text-white mb-2">{{ $product->title }}</h4>
                                        @if($product->short_description)
                                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ Str::limit($product->short_description, 60) }}</p>
                                        @endif
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <span class="text-lg font-bold text-purple-600 dark:text-purple-400">{{ $product->formatted_price }}</span>
                                                @if($product->is_on_sale)
                                                    <span class="text-sm text-gray-500 dark:text-gray-400 line-through ml-2">${{ number_format($product->compare_at_price_cents / 100, 2) }}</span>
                                                @endif
                                            </div>
                ...
                <form method="GET" action="{{ route('store.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <input type="text" name="search" placeholder="Search products..." value="{{ request('search') }}"
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <select name="category" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <div>
                        <select name="type" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('store.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition">
                            Clear
                        </a>
                    </div>
                </form>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                                ...
                                <div class="p-4">
                                    @if($product->is_featured)
                                        <span class="inline-block px-2 py-1 text-xs font-semibold text-pink-800 dark:text-pink-300 bg-pink-100 dark:bg-pink-900/50 rounded mb-2">Featured</span>
                                    @endif
                                    <h4 class="font-semibold text-gray-900 dark:text-white mb-2">{{ $product->title }}</h4>
                                    @if($product->category)
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">{{ $product->category->name }}</p>
                                    @endif
                                    @if($product->short_description)
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ Str::limit($product->short_description, 80) }}</p>
                                    @endif
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <span class="text-lg font-bold text-purple-600 dark:text-purple-400">{{ $product->formatted_price }}</span>
                                            @if($product->is_on_sale)
                                                <span class="text-sm text-gray-500 dark:text-gray-400 line-through ml-2">${{ number_format($product->compare_at_price_cents / 100, 2) }}</span>
                                                <span class="text-xs text-red-600 dark:text-red-400 font-semibold ml-2">-{{ $product->sale_percentage }}%</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="mt-2">
            ...
            @else
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-12 text-center">
                    ...
                    @if(request()->filled('search') || request()->filled('category') || request()->filled('type'))
                        <a href="{{ route('store.index') }}" class="mt-4 inline-block text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 font-semibold">
